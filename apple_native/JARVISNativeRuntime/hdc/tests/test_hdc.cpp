...
// ---------------------------------------------------------------------------
// Security: truncated blob must throw InvalidBlobLength, never read OOB
// (Run under ASan to verify no out-of-bounds access occurs.)
// ---------------------------------------------------------------------------
TEST_CASE("TernaryKernel: unpack_trits truncated blob throws, not OOB", "[ternary][security]") {
    TernaryKernel k(1024);
    // packed_bytes_ = ceil(1024/4) = 256; supply only 1 byte → InvalidBlobLength
    std::vector<uint8_t> short_blob(1, 0xAA);
    REQUIRE_THROWS_AS(k.unpack_trits(short_blob), InvalidBlobLength);
}

TEST_CASE("TernaryKernel: bind with truncated a_blob throws InvalidBlobLength", "[ternary][security]") {
    TernaryKernel k(64);
    auto good = k.zeros();                      // 16 bytes (64/4)
    std::vector<uint8_t> bad(4, 0u);            // too short
    REQUIRE_THROWS_AS(k.bind(bad, good), InvalidBlobLength);
    REQUIRE_THROWS_AS(k.bind(good, bad), InvalidBlobLength);
}

TEST_CASE("TernaryKernel: similarity with truncated blob throws InvalidBlobLength", "[ternary][security]") {
    TernaryKernel k(64);
    auto good = k.zeros();
    std::vector<uint8_t> bad(2, 0u);
    REQUIRE_THROWS_AS(k.similarity(bad, good), InvalidBlobLength);
    REQUIRE_THROWS_AS(k.similarity(good, bad), InvalidBlobLength);
}

TEST_CASE("TernaryKernel: bundle with one truncated blob throws InvalidBlobLength", "[ternary][security]") {
    TernaryKernel k(64);
    auto good = k.zeros();
    std::vector<uint8_t> bad(3, 0u);
    REQUIRE_THROWS_AS(k.bundle({good, bad}), InvalidBlobLength);
}
