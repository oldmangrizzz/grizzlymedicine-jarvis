 ...
 * from https://github.com/hsnyder/safetensors-cpp (included as a submodule under
 * voice/tts/libtorch/third_party/safetensors-cpp/).  If that submodule is not
 * present the CMake build will fall back to the Python-generated binary blob path
...
#else
// Fallback: raw binary read + torch::load (requires the file was previously
// converted with torch.save, see README §4).
