...
-----
    /path/to/.venv/bin/python3 trace_xtts_to_torchscript.py \\
        --out_dir /path/to/voice/tts/libtorch/models \\
        [--device mps|cuda|cpu]
...
def main() -> None:
    ...
    # ── Optional: dump oracle token sequences for byte-equiv test ────────────
    if args.dump_tokens:
...
def _dump_oracle_tokens(out_dir: Path, flow_lm: "FlowLMDecoder") -> None:
    ...
    if tok_model_path is None or not Path(tok_model_path).exists():
        ...
        for attr in ("tokenizer", "_tokenizer", "text_encoder"):
    ...
    oracle_prompts = Path(__file__).parent.parent.parent.parent.parent.parent.parent / \
                     "oracle" / "voice" / "prompts.json"
    if not oracle_prompts.exists():
        # Fallback search
        candidates = list(Path.home().glob("**/oracle/voice/prompts.json"))
        oracle_prompts = candidates[0] if candidates else oracle_prompts
