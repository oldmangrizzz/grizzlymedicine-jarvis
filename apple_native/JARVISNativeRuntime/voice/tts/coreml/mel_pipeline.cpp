    ...
    int n_bins)
{
    ...
    // frames and n_bins are upstream-controlled; overflow or over-capacity
    // must be detected before any read. Return NaN — caller MUST check.
    // §3 voice math sacrosanct: the float ops, windowing, filterbank, and
    // formulas below are UNCHANGED. Only the guard wrapper is new.
    ...
    (void)min_frames;  // min_frames used implicitly via frames_a/frames_b < max_frames
    return db;
}
