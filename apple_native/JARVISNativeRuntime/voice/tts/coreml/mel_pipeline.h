...
/// @param a_capacity_bytes Byte capacity of the mel_a buffer (sizeof(float)*frames*bins).
///                         Used to detect integer overflow before memcpy.
...
/// @return L2 distance in dB (lower is better, 0 = identical).
///         Returns NaN if an integer overflow is detected in the byte-count
///         computation or if bytes_needed > capacity; caller MUST check for NaN
///         and treat it as a security rejection (§2 AGENTS.md — no silent fallback).
float mel_l2_db(
