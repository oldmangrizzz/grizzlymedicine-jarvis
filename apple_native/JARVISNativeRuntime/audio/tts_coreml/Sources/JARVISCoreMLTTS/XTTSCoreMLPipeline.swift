...
/// via a thin C bridge. No fallback tokenizer is allowed for voice equivalence.
public class JARVISSentencePiece {
    ...
    public init(modelURL: URL) throws {
        ...
        if let h = jarvis_spm_load(modelURL.path) {
            ...
            throw TTSError.tokenizerError("SentencePiece model failed to load at \(modelURL.path); fallback tokenization is forbidden for voice equivalence")
        }
    }
    ...
    public func encode(_ text: String) throws -> [Int32] {
        guard let h = handle else {
            throw TTSError.tokenizerError("SentencePiece handle is not loaded")
        }
        var count: Int32 = 0
