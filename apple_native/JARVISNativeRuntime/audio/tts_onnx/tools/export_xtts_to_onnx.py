...
def attention_step_growing_cache(
    ...
    # Causal mask in absolute streaming positions.  This is required when the
    # first call batches text-conditioning tokens plus the first latent (or when
    # prompting text-only with a zero-length latent sequence): future tokens in
    # that batched query must not be visible.  Single-token decode degenerates
    # to an all-true mask and remains equivalent to the previous fast path.
    pos_k = torch.arange(new_kv_cache.shape[2], device=x.device, dtype=torch.long)
    pos_q = kv_len + torch.arange(T_q, device=x.device, dtype=torch.long)
    delta = pos_q.view(1, T_q, 1) - pos_k.view(1, 1, -1)
    attn_mask = delta >= 0
    if context is not None:
        attn_mask = attn_mask & (delta < context)
    attn_mask = attn_mask.unsqueeze(1).expand(B, 1, T_q, new_kv_cache.shape[2])
    out = F.scaled_dot_product_attention(q_t, k_t, v_t, attn_mask=attn_mask, dropout_p=0.0)
...
def _non_streaming_conv(layer, x: torch.Tensor) -> torch.Tensor:
    """Non-streaming pass through StreamingConv1d without beartype state init."""
...
def _seanet_decode_nonstreaming(decoder, emb_out: torch.Tensor) -> torch.Tensor:
    """Walk decoder.model, dispatching stateful streaming layers explicitly."""
    x = emb_out
...
def _mimi_transformer_full_seq(layer, rope: RotaryEmbedding,
    ...
    # Mimi's decoder transformer is causal with a finite context window in
    # pocket-tts.  `is_causal=True` alone lets every past frame attend, which
    # matches shapes but drifts audibly on long/full-sequence batch export.
    pos = torch.arange(T, device=x.device, dtype=torch.long)
    delta = pos.view(T, 1) - pos.view(1, T)
    attn_mask = delta >= 0
    if attn.context is not None:
        attn_mask = attn_mask & (delta < attn.context)
    attn_mask = attn_mask.view(1, 1, T, T).expand(B, 1, T, T)
    out = F.scaled_dot_product_attention(q_t, k_t, v_t, attn_mask=attn_mask, dropout_p=0.0)
...
def export_flow_lm_step(model: TTSModel, out_path: Path, opset: int):
    ...
    dynamic_axes = {
        "backbone_latent": {0: "batch", 1: "backbone_len"},
