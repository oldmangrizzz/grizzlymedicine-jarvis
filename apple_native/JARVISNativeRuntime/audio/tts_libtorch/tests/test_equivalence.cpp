...
TEST_CASE("mel_L2_equivalence_all_50_prompts", "[equivalence][libtorch]") {
    ...
    try {
        ...
        std::cout << "\n=== mel-L2 equivalence failure report ===\n";
        std::cout << "Root cause: TorchScript pipeline could not be loaded: " << e.what() << "\n";
        std::cout << "No synthesized PCM was produced; mel-L2 is NaN for every oracle prompt.\n";
        std::cout << " idx  mel_L2_dB  result\n";
        for (const auto& prompt : prompts) {
            std::cout << "  " << std::setfill('0') << std::setw(2) << prompt.idx
                      << std::setfill(' ') << "   NaN       FAIL\n";
        }
        FAIL("LibTorch TTS hard gate failed before inference; see per-prompt NaN report above");
    }
