...
std::vector<uint8_t> RealKernel::encode_scalar(
    float scaled,
    ...
    std::span<const float> basis) const
{
    ...
    if (rows == 0 || basis.size() != rows * static_cast<size_t>(dim_))
        throw std::invalid_argument("RealKernel::encode_scalar: basis shape mismatch");
    ...
    for (int d = 0; d < dim_; ++d) {
        ...
        out[static_cast<size_t>(d)] = std::tanh(acc);
    }
    return pack_floats(out);
}
