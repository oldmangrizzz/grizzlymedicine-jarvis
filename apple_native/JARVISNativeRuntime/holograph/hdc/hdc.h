...
public:
    ...
    /// PSP-HDC scalar encoder: phi((scaled * embedding)^T basis).
    /// `basis` is row-major with embedding.size() rows and dim() columns.
    virtual std::vector<uint8_t> encode_scalar(
        float scaled,
        std::span<const float> embedding,
        std::span<const float> basis) const = 0;
